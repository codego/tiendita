...
If Web Share or the clipboard fallback fails, do not fail silently: cream toast **No se pudo compartir. Probá de nuevo.** + **Reintentar**. Cancelled share stays quiet.
...
**20:55** (America/Buenos_Aires, once per day): **Está pasando en Con pinta.** / **20 minutos.** Tap always opens `/las21` — never the home feed. If the drop is on, that is the live stage. If there is no drop tonight (never started / floor not met / still before 21:00), cream empty: **Hoy no hay Las 21.** / **Volvé mañana a las 21.** / **Ir al feed** → `/`. If the drop already ran and the 21:00–21:20 window closed, stay on `/las21` with a soft banner **Se terminó Las 21.** and **Ir al feed** — not the empty night line. If the PWA or tab is alive and permission is granted, the client fires a local notification. Full server Web Push needs VAPID keys (see Env). Same minute, merchants with a piece in tonight’s Las 21 drop get **Tu pieza está en Las 21. 20 minutos.** — HTML + plain in `emails/las21-merchant.*`, CTA **Ver en el panel** → `/marcas`. Same `CONTACT_FROM` as Contacto / click mail (joacoditoma path). Never an invented brand mailbox. If that store is not in the drop, do not send. No merchant push opt-in yet — the hook is stubbed with the same copy. QA hooks: `/?avisame=1` (Avisame sheet), `/?ping=1` (fire the 20:55 notification now), `/?mail=las21` or `/marcas?mail=las21` (merchant 20:55 mail now), `/las21?drop=1` (force live), `npm run mail:las21` (`--force` outside the minute).
...
| `/las21` | Las 21. Live drop when it is on. No drop tonight: cream empty **Hoy no hay Las 21.** / **Volvé mañana a las 21.** / **Ir al feed**. After the window closed: soft banner **Se terminó Las 21.** + **Ir al feed**. WhatsApp OG stays “Está pasando en Con pinta. 20 minutos.” |
| `/marcas` | Brand gate if no session (**Conectá tu TiendaNube.**); merchant panel if session exists. Real OAuth if TN env is set; otherwise labeled mock. QA last-drop aviso: `?aviso=0` / `?aviso=47`. QA merchant 20:55 mail: `?mail=las21` |
...
3. **`/marcas/dashboard`** — Redirects to the panel home `/marcas`. Click notice for mail still: **Alguien salió de Con pinta a tu ficha.** Las 21 merchant notice (20:55, only if that store has a piece in tonight’s drop): **Tu pieza está en Las 21. 20 minutos.** CTA **Ver en el panel** → `/marcas`. Footer: **Solo te avisamos si tu pieza entra al drop. No es un newsletter.**
...
Without VAPID, **Avisame a las 21.** still asks `Notification.requestPermission`, and a granted PWA/tab can fire the 20:55 local notification (once per day). `POST /api/las21/push` is the documented server hook — it returns **501** until keys (and a subscription store) exist. The same 20:55 ping also POSTs `/api/las21/merchant-mail` for brands in tonight’s drop (once per ART day). Service worker click always opens `/las21` (live if the drop is on; empty **Hoy no hay Las 21.** if it never started; soft **Se terminó Las 21.** if the window already closed; then **Ir al feed**).
...
| `npm run lint` | ESLint |
| `npm run mail:las21` | POST the merchant 20:55 notice (needs the app). Add `--force` outside 20:55 ART |
