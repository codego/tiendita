...
import { usePublishedCount } from "@/lib/usePublished";
import { useLiveClickMap, useLiveStoreClicks } from "@/lib/useStoreClicks";

export function BrandDashboard() {
  const published = usePublishedCount();
  const liveClicks = useLiveStoreClicks();
  const clickMap = useLiveClickMap();
  ...
  const ranking = dashboardRanking.map((row) => {
    ...
    return {
      ...row,
      sku,
      clicks: row.clicks + (clickMap[row.skuId] ?? 0),
    };
  });

  const metrics = [
    {
      value: formatGrouped(dashboardMetrics.visits),
      label: dashboardCopy.visitsLabel,
    },
    {
      value: formatGrouped(dashboardMetrics.storeClicks + liveClicks),
      label: dashboardCopy.clicksLabel,
    },
    {
      value: formatGrouped(published),
