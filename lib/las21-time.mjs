...
export function isLas21Ended(now, storeCount = Number.POSITIVE_INFINITY) {
  if (!meetsLas21Floor(storeCount)) return false;
  if (isInLas21Window(now)) return false;
  return now >= todayWindowStartMs(now) + WINDOW_MS;
}
...
export function isForcePingParam(ping) {
  return ping === "1";
}

export function isForceMerchantMailParam(mail) {
  return mail === "las21";
}

export function pingDayKey(now) {
  const parts = zonedParts(now);
  return `${parts.year}-${String(parts.month).padStart(2, "0")}-${String(parts.day).padStart(2, "0")}`;
}

export function todayPingMs(now) {
  const parts = zonedParts(now);
  return zonedDateToUtcMs(
    parts.year,
    parts.month,
...
export function nextPingMs(now) {
  const today = todayPingMs(now);
  if (now < today) return today;
  const tomorrow = zonedParts(today + 24 * 60 * 60 * 1000);
  return zonedDateToUtcMs(
    tomorrow.year,
    tomorrow.month,
    tomorrow.day,
...
export function isInPingWindow(now) {
  const today = todayPingMs(now);
  return now >= today && now < today + PING_WINDOW_MS;
}

export function msUntilNextPing(now) {
  if (isInPingWindow(now)) return 0;
  return Math.max(0, nextPingMs(now) - now);
}
